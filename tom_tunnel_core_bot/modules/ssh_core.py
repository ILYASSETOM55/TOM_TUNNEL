...
def create_ssh_account(
    ...
    if not _valid_user(user):
        return False, (
            ...
            "Utilisez lettres, chiffres, point, tiret ou "
            "underscore (1-32 caractères)."
        )
    ...
    if _user_exists(user):
        return False, (
            f"❌ L'utilisateur "
            f"<code>{_esc(user)}</code> existe déjà."
        )
    ...
    try:
        ...
        return False, (
            "⚠️ Compte Linux créé mais métadonnées "
    ...
    # La logique de création ci-dessus reste inchangée.
    # ==========================================================
    ...
    return True, (
        ...
        "🔌 <b>Ports:</b> "
        "SSH 22 | Dropbear 109/143 | "
        "WS 8880/443 | UDP-CUSTOM 1-65535\n"

        "▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬\n"

        f"🐌 <b>SlowDNS PUB:</b>\n"
        f"<code>{_esc(pub)}</code>\n"
    ...
    for directory in (
        ...
        if directory.is_dir():
            ...
            for path in directory.glob("*.txt"):
                ...
                if (
                    _valid_user(path.stem)
                    and _user_exists(path.stem)
                ):
                    users.add(path.stem)
...
def get_ssh_account_details(user):
    ...
    return True, (
        ...
        f"📌 <b>Status:</b> "
        f"<code>{_account_status(user)}</code>\n"
...
def lock_ssh_account(user):
    ...
    return True, (
        f"🔒 <b>Compte "
        f"<code>{_esc(user)}</code> verrouillé.</b>"
    )
...
def unlock_ssh_account(user):
    ...
    return True, (
        f"🔓 <b>Compte "
        f"<code>{_esc(user)}</code> déverrouillé.</b>"
    )